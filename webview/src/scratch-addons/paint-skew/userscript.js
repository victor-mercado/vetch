...
export default async function ({ addon }) {
  ...
  const addSkew = function () {
    const selectTool = paper.tools.find(t => t.boundingBoxTool);
    if (!selectTool) return;
    // ScaleTool
    const ST = selectTool.boundingBoxTool._modeMap?.SCALE?.constructor;
    if (!ST || ST.hasSkew) {
      console.log("pSVG Skew Addon: not initializing. ST:", ST, "hasSkew:", ST?.hasSkew);
      return;
    }
    ST.hasSkew = true;
    console.log("pSVG Skew Addon: Successfully hooked into ScaleTool!");
    ...
    addons.paintSkew = function (event, original) {
      ...
      if ((event.modifiers.control || event.modifiers.command) && !this.isCorner) {
        console.log("pSVG Skew Addon: user is skewing!");
        // Skew
        // Reset position
        this.centered = false;
